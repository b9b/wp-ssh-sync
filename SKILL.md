...
- 修改远端数据前，先明确源目录、目标目录、同步方向，以及是否已经执行 dry-run。
...
- 除非用户明确要求真实同步，并且必要凭据和配置都已存在，否则不要修改生产数据。
...
```bash
scripts/sync-directories.sh --project-root /path/to/target-project --apply
```

只有当用户希望删除远端多余文件时，才添加 `--delete`。
